...
public abstract class GroovySyntaxTag implements GrailsTag {
    ...
    protected String calculateExpression(String expr) {
        Assert.isTrue(!StringUtils.isEmpty(expr), "Argument [expr] cannot be null or blank");
    ...
    protected void doEachMethod(String in) {
        ...
        boolean hasStatus = !StringUtils.isEmpty(status);
        boolean hasVar = !StringUtils.isEmpty(var);
        if (hasStatus && !hasVar) {
            throw new GspTagException(ERROR_NO_VAR_WITH_STATUS, parser.getPageName(), parser.getCurrentOutputLineNumber());
        }

        if (var.equals(status) && (hasStatus)) {
            throw new GspTagException("Attribute [" + ATTRIBUTE_VAR +
