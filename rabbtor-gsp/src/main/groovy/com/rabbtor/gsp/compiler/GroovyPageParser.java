...
import com.rabbtor.gsp.compiler.tags.GroovySyntaxTag;
import com.rabbtor.taglib.GspTagException;
import com.rabbtor.gsp.compiler.tags.GspTagRegistry;
import com.rabbtor.io.IOUtils;
import com.rabbtor.io.SpringIOUtils;
...
public class GroovyPageParser implements Tokens
{
    ...
    private GspTagRegistry tagRegistry = GspTagRegistry.getInstance();
    ...
    public static final String[] DEFAULT_IMPORTS = {
            "com.rabbtor.gsp.GroovyPage",
            "com.rabbtor.web.taglib.*",
            "com.rabbtor.taglib.GspTagException",
            "org.springframework.web.util.*"
    };
    public static final String CONFIG_PROPERTY_DEFAULT_CODEC = "default.codec";
    public static final String CONFIG_PROPERTY_GSP_ENCODING = "encoding";
    public static final String CONFIG_PROPERTY_GSP_KEEPGENERATED_DIR = "keepgenerateddir";
    public static final String CONFIG_PROPERTY_GSP_SITEMESH_PREPROCESS = "sitemesh.preprocess";
    public static final String CONFIG_PROPERTY_GSP_CODECS = "codecs";
    ...
    private static final String EXPRESSION_CODEC_DIRECTIVE = DefaultOutputEncodingSettings.EXPRESSION_CODEC_NAME + CODEC_DIRECTIVE_POSTFIX;
    private static final String EXPRESSION_CODEC_DIRECTIVE_ALIAS = "default" + CODEC_DIRECTIVE_POSTFIX;
    private static final String STATIC_CODEC_DIRECTIVE = DefaultOutputEncodingSettings.STATIC_CODEC_NAME + CODEC_DIRECTIVE_POSTFIX;
    private static final String OUT_CODEC_DIRECTIVE = DefaultOutputEncodingSettings.OUT_CODEC_NAME + CODEC_DIRECTIVE_POSTFIX;
    private static final String TAGLIB_CODEC_DIRECTIVE = DefaultOutputEncodingSettings.TAGLIB_CODEC_NAME + CODEC_DIRECTIVE_POSTFIX;
    ...
    private void page()
    {
        ...
        if (finalPass)
        {
            if (!tagMetaStack.isEmpty())
            {
                throw new GspTagException("Grails tags were not closed! [" +
    ...
    private void endTag()
    {
        ...
        {
            throw new GspTagException("Grails tag [" + lastNamespaceInStack +
        ...
        if (GroovyPage.DEFAULT_NAMESPACE.equals(ns) && tagRegistry.isSyntaxTag(tagName))
        {
            ...
            {
                throw new GspTagException("Grails tag [" + tagName +
        ...
        } else
        {
            ...
            if (jspTags.containsKey(ns))
            {
                ...
                out.println("if (!jspTag) throw new GspTagException('Unknown JSP tag " +
                        ns + ":" + tagName + "')");
    ...
    private void startTag()
    {
        ...
        if (GroovyPage.DEFAULT_NAMESPACE.equals(ns) && tagRegistry.isSyntaxTag(tagName))
        {
            ...
            {
                throw new GspTagException("Tag [" + tag.getName() +
    ...
    private void populateMapWithAttributes(Map<String, String> attrs, String attrTokens)
    {
        ...
        while (startPos < attrTokens.length())
        {
            ...
            if (equalsignPos == -1)
            {
                throw new GspTagException("Expecting '=' after attribute name (" + attrTokens + ").", pageName, getCurrentOutputLineNumber());
            }
            ...
            if (!(ch == '\'' || ch == '"'))
            {
                throw new GspTagException("Attribute value must be quoted (" + attrTokens + ").", pageName, getCurrentOutputLineNumber());
            }
            char quoteChar = ch;
            ...
            if (endQuotepos == -1)
            {
                throw new GspTagException("Attribute value quote wasn't closed (" + attrTokens + ").", pageName, getCurrentOutputLineNumber());
            }
