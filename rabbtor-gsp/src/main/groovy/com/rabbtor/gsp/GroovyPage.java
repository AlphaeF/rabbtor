...
import com.rabbtor.gsp.jsp.JspTag;
import com.rabbtor.gsp.jsp.JspTagLib;
import com.rabbtor.gsp.jsp.TagLibraryResolver;
import com.rabbtor.taglib.*;
import com.rabbtor.taglib.encoder.OutputContext;
import com.rabbtor.taglib.encoder.OutputEncodingStack;
import com.rabbtor.taglib.encoder.OutputEncodingStackAttributes;
...
public abstract class GroovyPage extends Script
{
    ...
    public void initRun(Writer target, OutputContext outputContext, GroovyPageMetaInfo metaInfo) {
        ...
        if (outputContext != null) {
            ...
            ApplicationContext applicationContext = outputContext.getApplicationContext();
            if (applicationContext != null) {
                rawEncoder = WithCodecHelper.lookupEncoder("Raw",applicationContext);
            }
        }
            ...
            if (tagLib != null || (gspTagLibraryLookup != null && gspTagLibraryLookup.hasNamespace(tagNamespace))) {
                if (tagLib != null) {
                    ...
                    if (tagLibClosure instanceof Closure) {
                        ...
                        throw new GspTagException("Tag [" + tagName + "] does not exist in tag library [" + tagLib.getClass().getName() + "]", getGroovyPageFileName(), lineNumber);
                    }
                } else {
                    throw new GspTagException("Tag [" + tagName + "] does not exist. No tag library found for namespace: " + tagNamespace, getGroovyPageFileName(), lineNumber);
                }
            } else {
    ...
    private void invokeTagLibClosure(String tagName, String tagNamespace, Closure<?> tagLibClosure, Map<?, ?> attrs, Closure<?> body,
        ...
        try {
            ...
            if (codecSettings != null) {
                outputStack.push(WithCodecHelper.createOutputStackAttributesBuilder(codecSettings, outputContext.getApplicationContext()).build());
    ...
    private void throwRootCause(String tagName, String tagNamespace, int lineNumber, Throwable e) {
        ...
        if (cause instanceof GspTagException) {
            // catch and rethrow with context
            throw new GspTagException(cause.getMessage(), getGroovyPageFileName(), lineNumber);
        }

        throw new GspTagException("Error executing tag <" + tagNamespace + ":" + tagName +
