...
public class WebUtils extends org.springframework.web.util.WebUtils {
    ...
    {
        RabbtorWebRequest webRequest = RabbtorWebRequest.lookup(request);
    ...
    private static boolean appendEntry(Map.Entry entry, StringBuilder queryString, String encoding, String path) throws UnsupportedEncodingException {
        ...
        if (value == null) value = "";
            ...
            RequestParams child = (RequestParams) value;
            Set nestedEntrySet = child.asMap().entrySet();
     ...
     * request.
     */
    public static RabbtorWebRequest retrieveRabbtorWebRequest() {
        RequestAttributes attributes = RequestContextHolder.currentRequestAttributes();
        if(attributes instanceof RabbtorWebRequest) {
            return (RabbtorWebRequest) attributes;
        }
        return (RabbtorWebRequest) attributes.getAttribute(RabbtorWebRequest.WEB_REQUEST,RequestAttributes.SCOPE_REQUEST);
    }
     ...
     */
    public static void storeRabbtorWebRequest(RabbtorWebRequest webRequest, boolean replaceRequestAttributes) {
        if (replaceRequestAttributes)
            RequestContextHolder.setRequestAttributes(webRequest);
        webRequest.setAttribute(RabbtorWebRequest.WEB_REQUEST, webRequest, RequestAttributes.SCOPE_REQUEST);
    }

    /**
     * Removes any GrailsWebRequest instance from the current request.
     */
    public static void clearRabbtorWebRequest() {
        RequestAttributes reqAttrs = RequestContextHolder.getRequestAttributes();
        if (reqAttrs != null) {
            // First remove the web request from the HTTP request attributes.
            RabbtorWebRequest webRequest = (RabbtorWebRequest) reqAttrs;
            webRequest.getRequest().removeAttribute(RabbtorWebRequest.WEB_REQUEST);
    ...
    public static String getForwardURI(HttpServletRequest request) {
        ...
        if (StringUtils.isEmpty(result)) result = request.getRequestURI();
        return result;
    }
    ...
    public static String addViewPrefix(String viewName) {
        return addViewPrefix(viewName, null);
    }
