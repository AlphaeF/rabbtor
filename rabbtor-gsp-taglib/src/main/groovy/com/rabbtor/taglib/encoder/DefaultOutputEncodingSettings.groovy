...
@CompileStatic
class DefaultOutputEncodingSettings implements OutputEncodingSettings
{
    ...
    public static final String CONFIG_PROPERTY_GSP_CODECS = "gsp.codecs"

    public static final String CONFIG_PROPERTY_DEFAULT_CODEC = "gsp.default.codec"
    ...
    DefaultOutputEncodingSettings()
    {
    }

    DefaultOutputEncodingSettings(Environment environment) {
    ...
    String getCodecSettings(String codecWriterName) {
        ...
        if (environment != null)
        {
            if (environment.hasProperty("${CONFIG_PROPERTY_GSP_CODECS}.${codecWriterName}"))
                codecInfo = environment.getProperty("${CONFIG_PROPERTY_GSP_CODECS}.${codecWriterName}")
            else
            {
                if (codecWriterName == EXPRESSION_CODEC_NAME)
                {
                    codecInfo = environment.getProperty(CONFIG_PROPERTY_DEFAULT_CODEC)?.toString()
                }
            }
        }
