...
class WithCodecHelper {
    private static final Log log = LogFactory.getLog(WithCodecHelper)
     ...
     */
    static <T> T withCodec(ApplicationContext applicationContext,Object codecInfo,Closure<T> closure) {
        OutputEncodingStack outputStack=OutputEncodingStack.currentStack()
        try {
            outputStack.push(createOutputStackAttributesBuilder(codecInfo,applicationContext).build(), false)
            return closure.call()
        } finally {
            outputStack.pop()
        }
     ...
     */
    static OutputEncodingStackAttributes.Builder createOutputStackAttributesBuilder(Object codecInfo, ApplicationContext applicationContext) {
        ...
        if (codecInfo != null) {
            ...
            codecInfoMap.each { String codecWriterName, Object codecName ->
                if (codecName instanceof String && !encoders.containsKey(codecName)) {
                    ...
                    encoders.put(codecNameString, lookupEncoder(codecNameString,applicationContext))
                }
            }
            builder.outEncoder(lookupEncoderFromMap(encoders, (String)codecInfoMap.get(DefaultOutputEncodingSettings.OUT_CODEC_NAME)))
            builder.taglibEncoder(lookupEncoderFromMap(encoders, (String)codecInfoMap.get(DefaultOutputEncodingSettings.TAGLIB_CODEC_NAME)))
            builder.expressionEncoder(lookupEncoderFromMap(encoders, (String)codecInfoMap.get(DefaultOutputEncodingSettings.EXPRESSION_CODEC_NAME)))
            builder.staticEncoder(lookupEncoderFromMap(encoders, (String)codecInfoMap.get(DefaultOutputEncodingSettings.STATIC_CODEC_NAME)))
            if (codecInfoMap.containsKey(DefaultOutputEncodingSettings.INHERIT_SETTING_NAME)) {
                builder.inheritPreviousEncoders(codecInfoMap.get(DefaultOutputEncodingSettings.INHERIT_SETTING_NAME) as boolean)
            }
            if (codecInfoMap.containsKey(DefaultOutputEncodingSettings.REPLACE_ONLY_SETTING_NAME)) {
                builder.replaceOnly(codecInfoMap.get(DefaultOutputEncodingSettings.REPLACE_ONLY_SETTING_NAME) as boolean)
            }
        }
        return builder
    }
    ...
    static Map<String, Object> makeSettingsCanonical(codecInfo) {
        ...
        if (codecInfo instanceof Map) {
            ...
            (Map<String, String>)((Map)codecInfo).each { k, v ->
                ...
                if (codecWriterName == DefaultOutputEncodingSettings.INHERIT_SETTING_NAME || codecWriterName == DefaultOutputEncodingSettings.REPLACE_ONLY_SETTING_NAME) {
                    codecInfoMap.put(codecWriterName, convertToBoolean(v))
                } else {
                    String codecName=v?.toString() ?: 'none'
                    if (DefaultOutputEncodingSettings.VALID_CODEC_SETTING_NAMES.contains(codecWriterName)) {
            ...
            if (allFallback || nameFallback) {
                for(String codecWriterName : DefaultOutputEncodingSettings.VALID_CODEC_SETTING_NAMES) {
                    String codecName=codecInfoMap.get(codecWriterName)?.toString()
                    if (!codecName) {
                        if (nameFallback && codecWriterName != DefaultOutputEncodingSettings.STATIC_CODEC_NAME) {
            ...
            for(String codecWriterName : DefaultOutputEncodingSettings.VALID_CODEC_SETTING_NAMES) {
                if (codecWriterName != DefaultOutputEncodingSettings.STATIC_CODEC_NAME) {
                    codecInfoMap.put(codecWriterName, codecName)
                }
            }
        }
    ...
    static Encoder lookupEncoder(String codecName,ApplicationContext applicationContext) {
